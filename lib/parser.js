...
var Command = {
  build: function (plainMessage) {
    ...
    if (this.isUnsub(message)) {
      movieName = this.extractMovieNameForUnsubscription(message);
  ...
  isInvalid: function (message) {
    var movie;
    if (this.isUnsub(message)) {
      movie = this.convertToTag(this.extractMovieNameForUnsubscription(message));
    } else {
  ...
  extractMovieNameForUnsubscription: function (message) {
